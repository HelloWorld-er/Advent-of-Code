	...
	while index < len(generated_list):
		...
		if generated_list[index] == '#':
			...
			for symbol_index in range(index, len(symbol_format_info[case_index])):
				...
				if symbol_format_info[case_index][symbol_index] == '#' or symbol_format_info[case_index][
					symbol_index] == '?':
					...
					if symbol_index > 0 and symbol_format_info[case_index][symbol_index - 1] == '#':
						break
					if symbol_index + number_format_info[case_index][layer] < len(symbol_format_info[case_index]) and \
							symbol_format_info[case_index][symbol_index + number_format_info[case_index][layer]] == '#':
						continue
					
					temp_num = 0
					temp_bool = False
					for element_index in range(symbol_index + number_format_info[case_index][layer],
					                           len(symbol_format_info[case_index])):
						if sharp_position[element_index] is True:
							if temp_bool is False:
								temp_bool = True
								temp_num += 1
						else:
							if temp_bool is True:
								temp_bool = False
					if len(number_format_info[case_index]) - layer - 1 < temp_bool:
						break

...
def recursion(index, case_index, layer):
	global symbol_format_info, generated_list, sharp_position, unknown_position, group_position, group_dict
	...
	while True:
		if sharp_position[index + 1][0] - sharp_position[index][1] < 2:
			break
		if sharp_position[index][0] != 0 and sharp_position[index][0] - 1 == '#':
			break
		...
		if group_position[group][1] - sharp_position[index][1] > 0:
			for element_index in range(index, len(sharp_position)):
				sharp_position[element_index][0] += 1
				sharp_position[element_index][1] += 1
				if element_index != len(sharp_position) - 1 and sharp_position[element_index + 1][0] - sharp_position[element_index][1] >= 2:
					break
		else:
